...
export default async function handler(req, res) {
  ...
  try {
    ...
    messageContent.push({
      type: "text",
      text: `이 패션 이미지를 보고 실제로 착용한 옷을 정확히 분석해주세요.
착용자: ${age} 여성, 체형: ${bodyType}

[중요 분석 규칙]
1. 실제로 몸에 입고 있는 옷만 분석하세요
2. 착용 방식을 정확히 구분하세요:
   - 어깨에 걸친 경우 → "어깨에 걸친 ○○"로 표현
   - 목/허리에 묶은 경우 → "목에 묶은 ○○" 또는 "허리에 묶은 ○○"로 표현
   - 손에 들고 있는 경우 → 분석에서 제외
   - 실제로 팔을 끼고 입은 경우에만 → 일반적인 아이템으로 표현
3. 색상을 반드시 정확하게 표현하세요 (회색/베이지/네이비 등 구체적으로)
4. 보이지 않는 아이템은 절대 추가하지 마세요

JSON만 반환 (코드블록 없이):
{
  "coreStyle": "이미지에서 보이는 핵심 스타일 2-3단어",
  "description": "실제 착용 룩의 특징 설명 2문장",
  "keyItems": ["실제 착용/걸친 아이템1", "아이템2", "아이템3"],
  "tip": "${bodyType} 체형을 위한 스타일링 팁 한 문장",
  "occasions": ["어울리는 상황1", "상황2"],
  "shopping": [
    {"item": "실제 착용 아이템명 (착용방식 포함)", "keyword": "색상+핏+아이템+여성 (예: 회색 루즈핏 가디건 여성)", "category": "카테고리"},
    {"item": "실제 착용 아이템명", "keyword": "색상+핏+아이템+여성", "category": "카테고리"},
    {"item": "실제 착용 아이템명", "keyword": "색상+핏+아이템+여성", "category": "카테고리"}
  ]
}`
    });
